--[[
    SANTES HUB v3.2 - Delta Executor Uyumlu (EQR Yükleme Mantığıyla)
    Tüm orijinal modüller aynen korunmuştur.
]]
...
local PlayerGui = LocalPlayer:WaitForChild("PlayerGui")  -- EQR Hub gibi bekle
...
-- ==================== TÜM MODÜLLER (ORİJİNAL SANTES HUB) ====================
-- (Aşağıdaki tüm modüller hiç değiştirilmeden orijinal haliyle bırakılmıştır.)
...
function NoFailLockpick_Enable()
    ...
    lockpickAddedConnection = pg.ChildAdded:Connect(function(Item)
...
local function formatName(name)
    ...
    local us = string.find(name, "_")
    if us then name = string.sub(name, 1, us-1) end
    return name
end
local function ApplyBredMakurzModification()
    ...
    for _, v in pairs(folder:GetChildren()) do
        local objPos
        if v.PrimaryPart and v.PrimaryPart:IsA("BasePart") then objPos = v.PrimaryPart.Position
        else local p = v:FindFirstChildOfClass("BasePart"); if p then objPos = p.Position else continue end end
        local dist = (objPos - pos).magnitude
        local existing = v:FindFirstChild("Ahh")
        if dist <= 200 then
            if not existing then
                local bg = Instance.new('BillboardGui')
                bg.Name = "Ahh"; bg.AlwaysOnTop = true; bg.Size = UDim2.new(8,0,4,0); bg.MaxDistance = 200; bg.Adornee = v; bg.Parent = v
                local txt = Instance.new('TextLabel')
                txt.Size = UDim2.new(1,0,1,0); txt.BackgroundTransparency = 1; txt.Font = Enum.Font.SourceSansBold; txt.TextSize = 15
                txt.Text = formatName(v.Name); txt.Parent = bg
                local values = v:FindFirstChild("Values")
                local broken = values and values:FindFirstChild("Broken")
                if broken then
                    if broken.Value ~= false then txt.TextColor3 = Color3.new(255,0,0) else txt.TextColor3 = Color3.new(0,255,0) end
                    broken:GetPropertyChangedSignal("Value"):Connect(function()
                        if broken.Value ~= false then txt.TextColor3 = Color3.new(255,0,0) else txt.TextColor3 = Color3.new(0,255,0) end
                    end)
                else txt.TextColor3 = Color3.new(0,255,0) end
            end
...
function SafeESP_Disable()
    ...
    local folder = Workspace.Map and Workspace.Map:FindFirstChild("BredMakurz")
    if folder then for _, v in pairs(folder:GetChildren()) do safeCall(function() if v:FindFirstChild("Ahh") then v.Ahh:Destroy() end end) end end
end
...
local function NearbyDoorInteraction_Loop()
    while (OpenNearbyDoors_Enabled or UnlockNearbyDoors_Enabled) do
        ...
        for _, door in pairs(doorsFolder:GetChildren()) do
            ...
            local valuesFolder = door:FindFirstChild("Values")
            local eventsFolder = door:FindFirstChild("Events")
            if doorBase and valuesFolder and eventsFolder then
                if (playerPos - doorBase.Position).Magnitude <= 6 then
                    ...
                    if UnlockNearbyDoors_Enabled then
                        ...
                        local lockArgument = door:FindFirstChild("Lock")
                    ...
                    if OpenNearbyDoors_Enabled then
                        ...
                        local knobArgument = door:FindFirstChild("Knob2") or door:FindFirstChild("Knob")
...
function AutoPickupMoney_Enable()
    ...
    AutoPickupMoney_Connection = RunService.RenderStepped:Connect(function()
        ...
        for _, v in pairs(cashFolder:GetChildren()) do
            if v:IsA("BasePart") and (rootPos - v.Position).Magnitude < 5 then
...
local AimBotSettings = {
    Enabled = false, TeamCheck = false, WallCheck = true, StickyAim = false, UseMouse = true,
    MouseBind = "MouseButton2", Keybind = nil, Fov = 100, Smoothing = 0.02,
...
local function CameraGetClosestToMouse_aim()
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= LocalPlayer and IsAlive_aim(v) then
            local char = v.Character
            local aimPart = char and char:FindFirstChild(AimBotSettings.AimPart)
            if aimPart then
                local aimPos = aimPart.Position
                local successWTV, screen_pos, on_screen = safeCall(function() return Workspace.CurrentCamera:WorldToViewportPoint(aimPos) end)
                if successWTV and on_screen then
                    local screen2D = Vector2.new(screen_pos.X, screen_pos.Y)
                    local mousePos = UserInputService:GetMouseLocation()
                    local mag = (screen2D - mousePos).Magnitude
                    if mag < AimFov and isVisible_aim(aimPos, char) then
                        AimFov = mag; targetPlayer = v
                    end
                end
            end
        end
...
UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
    ...
    if not AimBotSettings.UseMouse and AimBotSettings.Keybind and input.KeyCode == AimBotSettings.Keybind then
        AimBotSettings.Target = CameraGetClosestToMouse_aim(); AimBotSettings.IsAimKeyDown = true
...
UserInputService.InputEnded:Connect(function(input, gameProcessedEvent)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 then bind = "MouseButton1"
    elseif input.UserInputType == Enum.UserInputType.MouseButton2 then bind = "MouseButton2" end
    if (not AimBotSettings.UseMouse and AimBotSettings.Keybind and input.KeyCode == AimBotSettings.Keybind) or (AimBotSettings.UseMouse and bind == AimBotSettings.MouseBind) then
    ...
    if AimBotSettings and AimBotSettings.Enabled and AimBotSettings.IsAimKeyDown then
        ...
        if AimBotSettings.StickyAim then
            if currentTarget ~= nil and IsAlive_aim(currentTarget) then
                ...
                else AimBotSettings.Target = nil; if AimBotSettings.CameraTween then AimBotSettings.CameraTween:Cancel(); AimBotSettings.CameraTween = nil end end
            else
                local newTarget = CameraGetClosestToMouse_aim()
                AimBotSettings.Target = newTarget; currentTarget = newTarget
                ...
                elseif AimBotSettings.CameraTween then AimBotSettings.CameraTween:Cancel(); AimBotSettings.CameraTween = nil end
            end
        else
...
function Aimbot_Enable() AimBotSettings.Enabled = true end
function Aimbot_Disable() AimBotSettings.Enabled = false; AimBotSettings.IsAimKeyDown = false; AimBotSettings.Target = nil; if AimBotSettings.CameraTween then AimBotSettings.CameraTween:Cancel(); AimBotSettings.CameraTween = nil end end

--======================= INFINITE STAMINA =========================--
local isInfiniteStaminaEnabled = false
function InfiniteStamina_Enable() isInfiniteStaminaEnabled = true end
function InfiniteStamina_Disable() isInfiniteStaminaEnabled = false end

--======================= INVISIBILITY =========================--
local Shadow_Active = false
local Shadow_Usable = true
local Shadow_Char, Shadow_Hum, Shadow_HRP, Shadow_AnimTrack = nil, nil, nil, nil
local Shadow_Anim = Instance.new("Animation"); Shadow_Anim.AnimationId = "rbxassetid://215384594"
local Shadow_WarnGUI = Instance.new("ScreenGui"); Shadow_WarnGUI.Name = "ShadowWarningHUD"; Shadow_WarnGUI.Parent = CoreGui; Shadow_WarnGUI.ResetOnSpawn = false
local Shadow_WarnText = Instance.new("TextLabel", Shadow_WarnGUI)
Shadow_WarnText.Text = "⚠️ You are visible ⚠️"; Shadow_WarnText.Visible = false; Shadow_WarnText.Size = UDim2.new(0, 200, 0, 30); Shadow_WarnText.Position = UDim2.new(0.5, -100, 0.85, 0); Shadow_WarnText.BackgroundTransparency = 1; Shadow_WarnText.Font = Enum.Font.GothamSemibold; Shadow_WarnText.TextSize = 24; Shadow_WarnText.TextColor3 = Color3.fromRGB(255, 255, 0); Shadow_WarnText.TextStrokeTransparency = 0.5; Shadow_WarnText.ZIndex = 10
local function UpdateShadowRefs()
    Shadow_Char = LocalPlayer.Character; Shadow_Hum = Shadow_Char and Shadow_Char:FindFirstChildOfClass("Humanoid"); Shadow_HRP = Shadow_Char and Shadow_Char:FindFirstChild("HumanoidRootPart")
end
...
local function Shadow_Deactivate()
    ...
    if Shadow_Char then for _, v in pairs(Shadow_Char:GetDescendants()) do if v:IsA("BasePart") and v.Transparency == 0.5 then v.Transparency = 0 end end end
...
local function Shadow_Activate()
    ...
    if not Shadow_Char:FindFirstChild("Torso") then Shadow_Usable = false; return end
    Shadow_Active = true
...
RunService.Heartbeat:Connect(function(deltaTime)
    if not Shadow_Active or not Shadow_Usable then
        if not Shadow_Active and Shadow_Char then for _, v in pairs(Shadow_Char:GetDescendants()) do if v:IsA("BasePart") and v.Transparency == 0.5 then v.Transparency = 0 end end end
        Shadow_WarnText.Visible = false; return
    end
    if not Shadow_Char or not Shadow_Hum or not Shadow_HRP or not Shadow_Hum:IsDescendantOf(Workspace) or Shadow_Hum.Health <= 0 then Shadow_WarnText.Visible = false; return end
    Shadow_WarnText.Visible = not Shadow_CheckGrounded()
    local walk_speed = 12
    if Shadow_Hum.MoveDirection.Magnitude > 0 then Shadow_HRP.CFrame = Shadow_HRP.CFrame + (Shadow_Hum.MoveDirection * walk_speed * deltaTime) end
    local InitialCFrame = Shadow_HRP.CFrame; local InitialCamOffset = Shadow_Hum.CameraOffset
    ...
    if Shadow_Char then for _, v in pairs(Shadow_Char:GetDescendants()) do if v:IsA("BasePart") and v.Transparency ~= 1 then v.Transparency = 0.5 end end end
end)
LocalPlayer.CharacterAdded:Connect(function()
    if Shadow_Active then Shadow_Deactivate() end
    if Shadow_AnimTrack then safeCall(function() Shadow_AnimTrack:Stop() end); Shadow_AnimTrack = nil end
    task.wait(); UpdateShadowRefs()
    if Shadow_Hum and Shadow_Hum.RigType ~= Enum.HumanoidRigType.R6 then Shadow_Usable = false; if Shadow_Active then Shadow_Deactivate() end; return else Shadow_Usable = true end
    if Shadow_Active and Shadow_Usable then Shadow_Activate() end
end)
function Invis_Enable() Shadow_Activate() end
function Invis_Disable() Shadow_Deactivate() end

--======================= NO RECOIL =========================--
local NoRecoil_Enabled = false
function NoRecoil_Enable() NoRecoil_Enabled = true end
function NoRecoil_Disable() NoRecoil_Enabled = false end
...
local function isStaff(player)
    for _, uid in pairs(staffUsers) do if player.UserId == uid then return true end end
    return false
end
function AdminCheck_Enable()
    if AdminCheck_Enabled then return end
    AdminCheck_Enabled = true
    local function check(p) if p == LocalPlayer then return end; if isStaff(p) then LocalPlayer:Kick("SANTES: Staff - " .. p.Name) end end
...
function Ragebot_Enable()
    ...
    Ragebot_Coroutine = task.spawn(function() while Ragebot_Enabled do task.wait(0.1) end end)
end
...
local function createESP(player)
    ...
    local function setupESP(character)
        ...
        hl.Name = "SantesESP"; hl.FillColor = Color3.fromRGB(200, 20, 20); hl.FillTransparency = 0.6
        hl.OutlineColor = Color3.fromRGB(255, 50, 50); hl.OutlineTransparency = 0; hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop; hl.Parent = character
        local head = character:FindFirstChild("Head")
        if head then
            local bg = Instance.new("BillboardGui")
            bg.Name = "SantesESPInfo"; bg.Size = UDim2.new(0, 120, 0, 40); bg.StudsOffset = Vector3.new(0, 2.5, 0); bg.AlwaysOnTop = true; bg.Parent = head
            local nl = Instance.new("TextLabel"); nl.Size = UDim2.new(1, 0, 0.5, 0); nl.BackgroundTransparency = 1; nl.Text = player.Name; nl.TextColor3 = Color3.new(1, 1, 1); nl.Font = Enum.Font.GothamBold; nl.TextSize = 12; nl.Parent = bg
            local dl = Instance.new("TextLabel"); dl.Size = UDim2.new(1, 0, 0.3, 0); dl.Position = UDim2.new(0, 0, 0.7, 0); dl.BackgroundTransparency = 1; dl.TextColor3 = Color3.fromRGB(150, 150, 150); dl.Font = Enum.Font.Gotham; dl.TextSize = 10; dl.Parent = bg
            RunService.Heartbeat:Connect(function()
                local myChar = getCharacter()
                local myHrp = myChar and myChar:FindFirstChild("HumanoidRootPart")
                local tHrp = character:FindFirstChild("HumanoidRootPart")
                if myHrp and tHrp then dl.Text = string.format("%.1f m", (myHrp.Position - tHrp.Position).Magnitude) end
            end)
        end
    end
...
function ESP_Disable()
    ...
    espConnections = {}; espPlayers = {}
    for _, p in pairs(Players:GetPlayers()) do
        safeCall(function()
            if p.Character then
                local h = p.Character:FindFirstChild("SantesESP"); if h then h:Destroy() end
                for _, v in pairs(p.Character:GetDescendants()) do if v:IsA("BillboardGui") and v.Name == "SantesESPInfo" then v:Destroy() end end
            end
        end)
    end
end
...
glow.ImageColor3 = Color3.fromRGB(255, 0, 0); glow.ImageTransparency = 0.55
glow.Size = UDim2.new(1, 60, 1, 60); glow.Position = UDim2.new(0, -30, 0, -30); glow.ZIndex = 0

local titleBar = Instance.new("Frame")
titleBar.Name = "TitleBar"; titleBar.Size = UDim2.new(1, 0, 0, 34); titleBar.BackgroundColor3 = C.panel; titleBar.BorderSizePixel = 0; titleBar.Parent = panel
Instance.new("UICorner", titleBar).CornerRadius = UDim.new(0, 10)

local titleLabel = Instance.new("TextLabel")
titleLabel.BackgroundTransparency = 1; titleLabel.Position = UDim2.new(0, 14, 0, 0); titleLabel.Size = UDim2.new(1, -80, 1, 0)
titleLabel.Font = Enum.Font.GothamBold; titleLabel.Text = "SANTES HUB"; titleLabel.TextColor3 = C.accent; titleLabel.TextSize = 15; titleLabel.TextXAlignment = Enum.TextXAlignment.Left; titleLabel.Parent = titleBar

local minimizeButton = Instance.new("TextButton")
minimizeButton.Name = "MinimizeButton"; minimizeButton.BackgroundTransparency = 1; minimizeButton.Position = UDim2.new(1, -64, 0, 0)
minimizeButton.Size = UDim2.new(0, 30, 1, 0); minimizeButton.Font = Enum.Font.GothamBold; minimizeButton.Text = "—"; minimizeButton.TextColor3 = C.text3; minimizeButton.TextSize = 16; minimizeButton.Parent = titleBar

local closeButton = Instance.new("TextButton")
closeButton.BackgroundTransparency = 1; closeButton.Position = UDim2.new(1, -34, 0, 0); closeButton.Size = UDim2.new(0, 34, 1, 0)
closeButton.Font = Enum.Font.GothamBold; closeButton.Text = "×"; closeButton.TextColor3 = C.text3; closeButton.TextSize = 18; closeButton.Parent = titleBar
closeButton.MouseButton1Click:Connect(function()
    safeCall(function()
        if Fly_Enabled then Fly_Disable() end; if Noclip_Enabled then Noclip_Disable() end; if FullBright_Enabled then FullBright_Disable() end
        if espEnabled then ESP_Disable() end; if Shadow_Active then Shadow_Deactivate() end; if NoRecoil_Enabled then NoRecoil_Disable() end
        if autofarmEnabled then Autofarm_Disable() end; if AutoPickupMoney_Enabled then AutoPickupMoney_Disable() end
        if AdminCheck_Enabled then AdminCheck_Disable() end; if Ragebot_Enabled then Ragebot_Disable() end
        if MeleeAura_Enabled then MeleeAura_Disable() end; if AimBotSettings.Enabled then Aimbot_Disable() end
    end)
    screenGui:Destroy()
end)

local body = Instance.new("Frame")
body.Name = "Body"; body.Position = UDim2.new(0, 0, 0, 34); body.Size = UDim2.new(1, 0, 1, -34 - 42); body.BackgroundTransparency = 1; body.Parent = panel

local sidebar = Instance.new("Frame")
sidebar.Name = "Sidebar"; sidebar.Size = UDim2.new(0, 130, 1, 0); sidebar.BackgroundColor3 = C.panel; sidebar.BorderSizePixel = 0; sidebar.Parent = body
Instance.new("UIListLayout", sidebar).Padding = UDim.new(0, 2)

local content = Instance.new("ScrollingFrame")
content.Name = "Content"; content.Position = UDim2.new(0, 130, 0, 0); content.Size = UDim2.new(1, -130, 1, 0)
content.BackgroundTransparency = 1; content.BorderSizePixel = 0; content.ScrollBarThickness = 3; content.ScrollBarImageColor3 = C.accent
content.CanvasSize = UDim2.new(0, 0, 0, 0); content.Parent = body

local contentList = Instance.new("UIListLayout", content)
contentList.Padding = UDim.new(0, 8); contentList.SortOrder = Enum.SortOrder.LayoutOrder
...
local function createToggleRow(labelText, defaultOn, onEnable, onDisable, getState)
    ...
    row.Size = UDim2.new(1, 0, 0, 40); row.BackgroundColor3 = C.card; row.BorderSizePixel = 0; row.Parent = content
    Instance.new("UICorner", row).CornerRadius = UDim.new(0, 6)
    local rs = Instance.new("UIStroke", row); rs.Color = C.border; rs.Thickness = 1

    local label = Instance.new("TextLabel")
    label.BackgroundTransparency = 1; label.Position = UDim2.new(0, 14, 0, 0); label.Size = UDim2.new(1, -80, 1, 0)
    label.Font = Enum.Font.Gotham; label.Text = labelText; label.TextColor3 = C.text2; label.TextSize = 13; label.TextXAlignment = Enum.TextXAlignment.Left; label.Parent = row

    local toggleBg = Instance.new("TextButton")
    toggleBg.Name = "Toggle"; toggleBg.Position = UDim2.new(1, -54, 0.5, -10); toggleBg.Size = UDim2.new(0, 40, 0, 20)
    toggleBg.Text = ""; toggleBg.AutoButtonColor = false; toggleBg.BackgroundColor3 = defaultOn and C.on or C.off; toggleBg.Parent = row
    Instance.new("UICorner", toggleBg).CornerRadius = UDim.new(1, 0)

    local knob = Instance.new("Frame")
    knob.Size = UDim2.new(0, 16, 0, 16); knob.Position = defaultOn and UDim2.new(1, -18, 0.5, -8) or UDim2.new(0, 2, 0.5, -8)
    knob.BackgroundColor3 = Color3.fromRGB(240, 240, 240); knob.BorderSizePixel = 0; knob.Parent = toggleBg
    ...
    local function updateState()
        ...
        knob.Position = targetPos; toggleBg.BackgroundColor3 = targetColor
    end

    toggleBg.MouseButton1Click:Connect(function()
        if isOn then if onDisable then safeCall(onDisable) end
        else if onEnable then safeCall(onEnable) end end
        isOn = not isOn
        updateState()
    end)
    updateState()
...
for i, name in ipairs(tabNames) do
    ...
    tab.Size = UDim2.new(1, 0, 0, 36); tab.BackgroundTransparency = 1; tab.Font = Enum.Font.Gotham
    tab.Text = "   " .. name; tab.TextXAlignment = Enum.TextXAlignment.Left; tab.TextColor3 = C.text3; tab.TextSize = 13; tab.Parent = sidebar
    local indicator = Instance.new("Frame")
    indicator.Size = UDim2.new(0, 3, 1, 0); indicator.BackgroundColor3 = C.accent; indicator.BackgroundTransparency = 1; indicator.BorderSizePixel = 0; indicator.Parent = tab
    tabs[i] = {button = tab, indicator = indicator}
end

local function selectTab(index)
    for i, t in ipairs(tabs) do
        if i == index then t.button.TextColor3 = C.accent; t.indicator.BackgroundTransparency = 0
        else t.button.TextColor3 = C.text3; t.indicator.BackgroundTransparency = 1 end
    end
...
for i, t in ipairs(tabs) do
    t.button.MouseButton1Click:Connect(function() selectTab(i) end)
end
selectTab(1)

-- Footer
local footer = Instance.new("Frame")
footer.Position = UDim2.new(0, 0, 1, -42); footer.Size = UDim2.new(1, 0, 0, 42); footer.BackgroundColor3 = C.panel; footer.BorderSizePixel = 0; footer.Parent = panel
Instance.new("UICorner", footer).CornerRadius = UDim.new(0, 10)

local footerLabel = Instance.new("TextLabel")
footerLabel.BackgroundTransparency = 1; footerLabel.Position = UDim2.new(0, 14, 0, 0); footerLabel.Size = UDim2.new(1, -28, 1, 0)
footerLabel.Font = Enum.Font.Gotham; footerLabel.Text = LocalPlayer.Name; footerLabel.TextColor3 = C.text2; footerLabel.TextSize = 12; footerLabel.TextXAlignment = Enum.TextXAlignment.Left; footerLabel.Parent = footer

-- Sürükleme ve Minimize (aynen orijinal)
local dragging, dragInput, dragStart, startPos, wasDragged
wasDragged = false
titleBar.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        dragging = true; wasDragged = false; dragStart = input.Position; startPos = panel.Position
...
local function minimizePanel()
    isMinimized = true; body.Visible = false; footer.Visible = false; minimizeButton.Visible = false; closeButton.Visible = false
    titleLabel.Text = "SANTES\nHUB"; titleLabel.TextSize = 13; titleLabel.TextWrapped = true
    titleLabel.TextXAlignment = Enum.TextXAlignment.Center; titleLabel.TextYAlignment = Enum.TextYAlignment.Center
    titleLabel.Position = UDim2.new(0, 0, 0, 0); titleLabel.Size = UDim2.new(1, 0, 1, 0); titleBar.Size = UDim2.new(1, 0, 1, 0)
...
local function restorePanel()
    ...
    task.wait(0.18)
    titleBar.Size = UDim2.new(1, 0, 0, 34); titleLabel.Text = "SANTES HUB"; titleLabel.TextSize = 15; titleLabel.TextWrapped = false
    titleLabel.TextXAlignment = Enum.TextXAlignment.Left; titleLabel.TextYAlignment = Enum.TextYAlignment.Center
    titleLabel.Position = UDim2.new(0, 14, 0, 0); titleLabel.Size = UDim2.new(1, -80, 1, 0)
    body.Visible = true; footer.Visible = true; minimizeButton.Visible = true; closeButton.Visible = true
end
minimizeButton.MouseButton1Click:Connect(minimizePanel)
titleBar.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 and isMinimized and not wasDragged then restorePanel() end
    wasDragged = false
end)
